  ...
  const msg = {
    ...
    hu_HU: {
      ...
      options: {
        ...
        losses:
          "Küldjön támadást olyan falvakba ahol részleges veszteséggel járhat a támadás:",
        maxloot:
        ...
        $.when(buildOptions()).then((html) => {
          ...
          $(".optionButton")
            ...
            .on("click", () => {
              ...
              getData(optionGroup, optionNewbarbs, optionLosses).then(
                (data) => {
                  Dialog.close();
          ...
          if ($("#mobileHeader").length) {
            game_data.units.forEach((unit) => {
              if (skipUnits.indexOf(unit) == -1) {
                ...
                units.push(
                  $img.length ? $img.closest("tr").text().trim().toNumber() : 0
                );
              }
            });
          } else {
